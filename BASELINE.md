...
* **The long-short book is the one result that survives its own noise** — all
  four draws clear |t| = 2 (+2.38 … +2.97) — but it still fails the locked
  holdout on two independent artifacts (+1.66, +1.87, +1.95, +2.12), so it is a
...
|---|---|---|---|---|---|---|
| long-short | 17.79% | 1.09 | -13.98% | +0.24 | +9.28% | +2.52 |
...
against. At 1% the same panel gives CAGR 17.50%, alpha +9.03%, t +2.45: inside
the ±1.46% draw-to-draw spread on alpha, so it changes nothing that is claimed
here.
...
### A correction to the long-short figures

An earlier revision of this table published long-short at **18.04% CAGR,
+9.53% alpha, t +2.56**, and a spread of **17.22% ± 1.66%**. Those were measured
with a defect in the engine's unpriced-gap counter: `cummax()` skips NaN without
filling it, so the "sessions since this name last printed" frame was NaN on
exactly the rows the disposal branch reads. The gap therefore always came out as
*sessions since the start of the backtest*, and a holding that missed a single
session had any grace period already exhausted and was written off to zero.

On this artifact that turned **17 deferrals into write-offs**. Corrected, the
book reports 17.79% / +9.28% / t +2.52, and the four-draw spread moves to
17.08% ± 1.65%. Every draw still clears |t| = 2 and the conclusion is unchanged;
the figures are restated because they moved, not because the finding did. The
long-only engines are untouched — their disposal counts were 0 and 1.
...
|---|---|---|---|---|---|
| **long-short** | **17.08% ± 1.65%** | **1.06 ± 0.12** | **+8.82% ± 1.46%** | **+2.58 ± 0.27** | **+2.38 … +2.97** |
...
**Long-short.** Every one of the four draws clears |t| = 2 — +2.38, +2.44,
+2.52, +2.97 — on a beta that barely moves (0.227 ± 0.010) and the tightest
